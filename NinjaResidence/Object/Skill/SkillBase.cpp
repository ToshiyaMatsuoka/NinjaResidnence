...
bool SkillBase::CollisionRope()
{
	...
	m_ropeX = static_cast<int>((central.x - MapChip::Scroll().X) / CELL_SIZE);
	m_ropeY = static_cast<int>((central.y - MapChip::Scroll().Y) / CELL_SIZE);
...
bool SkillBase::CollisionTarget()
{
	...
	m_targetX = static_cast<int>((central.x - MapChip::Scroll().X) / CELL_SIZE);
	m_targetY = static_cast<int>((central.y - MapChip::Scroll().Y) / CELL_SIZE);
	return coll;
}
